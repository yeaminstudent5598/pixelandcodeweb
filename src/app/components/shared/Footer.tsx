    ...
    <footer className="relative bg-white dark:bg-black text-slate-600 dark:text-slate-400 border-t border-slate-200 dark:border-slate-800 overflow-hidden">
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center border-b border-slate-200 dark:border-slate-800 pb-16 mb-16 gap-8">
          <div className="max-w-2xl">
            ...
            <p className="text-lg text-slate-600 dark:text-slate-400">
              {language 
                ? 'আপনার আইডিয়া, আমাদের টেকনোলজি। সফলতার পথে যাত্রা শুরু হোক আজই।' 
                : 'Your idea, our technology. Let the journey to success begin today.'}
            </p>
          </div>
          
          <Link href="/contact" className="group relative inline-flex items-center justify-center px-8 py-4 text-lg font-bold text-white transition-all duration-200 bg-slate-900 dark:bg-white dark:text-black rounded-full hover:scale-105 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-slate-900">
      ...
      >
        ...
        />
              ...
              <span className="text-2xl font-black text-slate-900 dark:text-white tracking-tight">Pixel & Code</span>
            </Link>
            ...
            <p className="text-base leading-relaxed text-slate-500 dark:text-slate-400 max-w-sm">
              {language 
                ...
                : 'Empowering businesses with modern technology to ensure digital transformation.'}
            </p>
          ...
          <div className="lg:col-span-2">
            <h3 className="font-bold text-slate-900 dark:text-white mb-6">{language ? 'কোম্পানি' : 'Company'}</h3>
            <ul className="space-y-4">
              {['Home', 'About Us', 'Services', 'Portfolio', 'Contact'].map((item) => (
                <li key={item}>
                  <Link href={`/${item.toLowerCase().replace(' ', '-')}`} className="text-slate-500 dark:text-slate-400 hover:text-blue-600 dark:hover:text-blue-400 transition-colors text-sm">
                     {item}
                  </Link>
                </li>
              ))}
          ...
          <div className="lg:col-span-2">
            <h3 className="font-bold text-slate-900 dark:text-white mb-6">{language ? 'সার্ভিসসমূহ' : 'Services'}</h3>
            <ul className="space-y-4">
              {['Web Design', 'App Development', 'Digital Marketing', 'Graphics Design', 'SEO'].map((item) => (
                <li key={item}>
                  <Link href="/services" className="text-slate-500 dark:text-slate-400 hover:text-blue-600 dark:hover:text-blue-400 transition-colors text-sm">
                     {item}
                  </Link>
                </li>
              ))}
          ...
          <div className="lg:col-span-4 space-y-8">
            <div>
              ...
              <div className="relative">
                <input 
                  ...
                  className="w-full bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-full py-3 px-5 focus:outline-none focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 text-slate-900 dark:text-white placeholder:text-slate-400"
                />
                <button className="absolute right-1.5 top-1.5 w-10 h-10 bg-blue-600 text-white rounded-full flex items-center justify-center hover:bg-blue-700 transition-colors">
            ...
            <div className="space-y-3">
               <div className="flex items-start gap-3 text-slate-500 dark:text-slate-400 text-sm">
                 ...
                 <span>Shariatpur Sadar, Shariatpur, Bangladesh</span>
               </div>
               <div className="flex items-center gap-3 text-slate-500 dark:text-slate-400 text-sm">
                 <Phone className="w-5 h-5 text-blue-500 shrink-0" />
                 <span>+880 1989-415813</span>
               </div>
               <div className="flex items-center gap-3 text-slate-500 dark:text-slate-400 text-sm">
                 <Mail className="w-5 h-5 text-blue-500 shrink-0" />
                 <span>pixelandcode07@gmail.com</span>
               </div>
            </div>
          </div>
        ...
        <div className="flex flex-col md:flex-row justify-between items-center pt-8 border-t border-slate-200 dark:border-slate-800">
          ...
          <div className="flex gap-6 mt-4 md:mt-0 text-sm">
            <Link href="/privacy" className="text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white transition-colors">Privacy Policy</Link>
            <Link href="/terms" className="text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white transition-colors">Terms of Service</Link>
          </div>
        </div>
      ...
      <div className="absolute bottom-0 left-1/2 -translate-x-1/2 pointer-events-none opacity-[0.03] dark:opacity-[0.05] whitespace-nowrap select-none">
        <span className="text-[12rem] md:text-[18rem] font-black text-slate-900 dark:text-white leading-none">
          PIXEL&CODE
        </span>
      </div>
